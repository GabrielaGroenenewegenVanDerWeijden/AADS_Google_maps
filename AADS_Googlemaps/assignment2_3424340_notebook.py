...
class BFSSolverMultipleFastestPaths(BFSSolverFastestPath):
    ...
    def find_n_paths(self):
        ...
        # Initializing the list containing all the path founds with their respective costs.
        paths = []

        while len(self.found_destinations) < self.find_at_most and self.priorityqueue:
            current, cost = self.priorityqueue.pop(0)

            if self.base_case(current):
                continue

            # Otherwise, we are gonna go through all the next possible steps. Plus taking into account the speed_limit and the distance.
            for next_node, distance, speed_limit in self.graph[current]:
                self.step(current, next_node, distance, speed_limit)
                
        for destination in self.found_destinations:
            # Initializing path and cost for each destination found.
            path = []

            # For as long the node is not None it will be added to the path and the new current node is initialized using the dictionary of history.
            while destination is not None:
                print(self.history[destination])
                path.append(destination)
                destination = self.history[destination][0]

            # Reverse the path to get it from source to destination.
            path = path[::-1]

            # Getting the cost for the path.
            print(self.history[destination][1])
            cost = self.history[destination][1]
           
            print(path, cost)
            paths.append((path, cost))

            # Sorting the paths from fastest to slowest.
    ...
    def base_case(self, node):
        ...
        :rtype: bool
        """
        # Checking if the node is in the destination nodes and not already found.
        if node in self.destination_nodes and node not in self.found_destinations:
            self.found_destinations.append(node)
            # Returning true when we have reached are outmost paths.
            return len(self.found_destinations) == self.find_at_most
